...
function loggergg(...log) {
    loggggg += log.reduce((a, b) => a + b) + "\n"
}
class State {
    transitions = {};
    name = '';
...
class TuringMachine {
    ...
    setInitialState(state) {
        ...
        if (!found) {
            loggergg(`State ${state} used in initial states not found, exiting...`);
        }
    }
    ...
    setAcceptingStates(states) {
        for (const s of states) {
            ...
            if (!found) {
                loggergg(`State ${s} used in accepting states not found, exiting...`);
            }
        }
    }
        ...
        for (const s of this.states) {
            for (const t in s.transitions) {
                if (s.transitions[t].next && !states.includes(s.transitions[t].next)) {
                    loggergg(`State ${s.transitions[t].next} used in ${s.name} not found, exiting...`);
                }
            }
        }
    }
    ...
    printTape(padSize = 0) {
        loggergg(this.tape.join('') + "    @" + this.states[this.state].name + " ".repeat(padSize));
        loggergg(' '.repeat(this.head) + '^' + " ".repeat(padSize));
    }
    ...
    convertToMentor(appendHead) {
        ...
        // loggergg(mentorHeader + mentorStates)
        return mentorHeader + mentorStates
    }
}

export function conv(c, padding) {
    let content = load(c);
    let blank = ' '
    if (!content.blank)
        loggergg('No blank character specified, using default');
    else
        blank = content.blank;
    let tm = new TuringMachine(blank);
    if (!content.table) {
        loggergg('No transitions specified, exiting...');
    }
        ...
        for (const tarr in curr) {
            ...
            if (typeof (curr[tarr]) == "string") {
                if (!["L", "R"].includes(curr[tarr])) {
                    loggergg(`Invalid move direction for state ${state}, exiting...`);
                }
    ...
    if (!("start state" in content)) {
        loggergg('No initial state specified, exiting...');
    }
    if (!("accept states" in content))
        loggergg('Warning: No accepting states specified');
    else {
    ...
    return tm.convertToMentor(padding)
}
